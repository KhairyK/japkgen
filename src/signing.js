...
export function sanitizeAlias(input = "") {
  return String(input).trim().replace(/\s+/g, "_").replace(/[^A-Za-z0-9._-]/g, "_") || DEFAULTS.keystoreAlias;
}
...
export async function createKeystore({
  ...
  if (!result.ok) {
    throw new Error(result.stderr || result.stdout || "Gagal membuat keystore. Pastikan keytool tersedia.");
  }
  return { keystorePath: resolved, stdout: result.stdout, stderr: result.stderr };
}
